...
const testimonials = [
  {
    quote: "Since their start, ProForce 1 has delivered the highest quality of professional services.",
    author: "Daniel Brooks",
    role: "Director of Operations, Major Healthcare Provider"
  },
  {
    quote: "Their team responds instantly to any issue. We've never felt safer.",
    author: "Sarah Jenkins",
    role: "Property Manager, Luxury Retail Center"
  },
  {
    quote: "ProForce 1 transformed our construction site security. Theft dropped to zero.",
    author: "Michael Ross",
    role: "Project Manager, Urban Development Group"
  },
  {
    quote: "Professional, courteous, and always on time. They truly understand hospitality security.",
    author: "Elena Rodriguez",
    role: "General Manager, Boutique Hotel Chain"
  }
]
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-slate-900">
      ...
      */}
      ...
      {/* --- 2. TESTIMONIAL STRIP --- */}
      <section className="bg-white border-b border-slate-100 py-16 px-4 relative z-20 -mt-4 rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] overflow-hidden">
          ...
          <div className="relative h-40 md:h-32 flex items-center justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3 text-sm font-bold text-slate-500 uppercase tracking-widest">
                  <span className="text-slate-900">{testimonials[currentTestimonial].author}</span>
                  <span className="hidden md:inline text-red-300">|</span>
                  <span className="text-red-600 md:text-slate-500">{testimonials[currentTestimonial].role}</span>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="flex justify-center gap-2 mt-4">
            {testimonials.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentTestimonial ? "bg-red-600 w-6" : "bg-slate-300 hover:bg-red-300"
                  }`}
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input name="email" required type="email" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none transition-colors placeholder:text-slate-300" placeholder="name@company.com" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Phone Number *</label>
                  <input name="phone" required type="tel" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none transition-colors placeholder:text-slate-300" placeholder="(800) 779-7691" />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input name="firstName" required type="text" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none transition-colors" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Last Name *</label>
                  <input name="lastName" required type="text" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none transition-colors" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Property Address</label>
                <input name="propertyAddress" type="text" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none transition-colors" placeholder="Street, City, Zip Code" />
              </div>

              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Service Type *</label>
                  <select name="serviceType" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none cursor-pointer text-slate-700">
                ...
                <div className="space-y-2">
                  ...
                  <select name="duration" className="w-full pb-2 border-b-2 border-slate-200 bg-transparent focus:border-red-600 outline-none cursor-pointer text-slate-700">
              ...
              <div className="space-y-2">
                ...
                <textarea name="message" required rows={3} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-600 outline-none resize-none" placeholder="Describe your security needs..." />
              </div>

              <div className="pt-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3 block">RFP Upload (Optional)</label>
                <div
                  className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 hover:border-red-400 transition-all group relative"
                >
                  <input
                    type="file"
                    name="file"
                    ...
                    onChange={(e) => {
                      ...
                      const span = e.target.nextSibling?.nextSibling as HTMLSpanElement;
              ...
              <div className="pt-6">
                ...
                {/* Result Message */}
                {result && (
                  <p className={`mt-4 text-center font-bold ${result.success ? "text-green-600" : "text-red-600"}`}>
                    {result.message}
                  </p>
                )}
              </div>
